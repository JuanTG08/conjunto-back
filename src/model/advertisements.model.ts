...
class RolModel {
    ...
    static findOneById(_id: ObjectId) {
        return AdvertisementsPages.find({ _id })
            .then(resp => {
                if (resp.length > 0) return Hook.Message(false, 200, "Ok", resp);
                return Hook.Message(false, 501, "No se encontro nada.");
            }).catch(err => {
    ...
    static modify(data: any, _id: ObjectId) {
        return AdvertisementsPages.findByIdAndUpdate(_id, data)
            .then(resp => {
    ...
    static disable(_id: ObjectId) {
        return AdvertisementsPages.findByIdAndUpdate(_id, { status: false })
            .then(resp => {
    ...
    static delete(_id: ObjectId) {
        return AdvertisementsPages.findByIdAndRemove(_id)
            .then(resp => {
